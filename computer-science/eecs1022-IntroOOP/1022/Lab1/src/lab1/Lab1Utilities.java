...
public class Lab1Utilities {
    ...
    * 
	...
	public static String temperatureConverter(double fahrenheit) {
		...
		String result = "Fahrenheit ("+fahrenheit+") is equivalent to ("+ String.format("%.2f", celsius) +") in Celsius";
	...
	public static String ageInYearsDetailed(int days) {
		...
		if (dayss>=365) {
			year = dayss/365;
			dayss = dayss - year*365;
		} if (dayss>=30) {
			month = dayss/30;
			dayss = dayss - month*30;
			}
		String result = "Age in days (" + days + "), age in years is (" + year + "), months (" + month +"), with remaining days (" + dayss + ")";
		return result;
		}
	...
	public static String distanceConverter(double cm) {
		...
		double cmm = cm;
		int feet = 0;
		double inch = cmm/2.54;
		
		if (inch >= 12) {
			feet = (int)inch/12;
			inch = inch - feet*12;
		}
		String result = String.format("%.2f", cm) + "cm is " + feet + "ft and " + String.format("%.2f", inch) + "in";
	...
	public static String fiveYearBalance(double balance, double interest) {
		...
		double balancee = balance;
		double interestRate = interest/100 + 1;
		
		for (int i = 1; i<=5; i++) {
			balancee = balancee*interestRate;
		}
		String result = "The balance was " + String.format("%.2f", balance) +" with interest " + String.format("%.2f", interest) + ", after five years balance is " + String.format("%.2f", balancee);
		return result;
	}
	...
	public static double computeBMI(int pounds, int feet, int inches) {
		...
		return Double.parseDouble(BMII);
	}
    ...
    * </p>
	...
	public static String isPerfectSquare(int n) {
		...
		double check = Math.floor(square);
