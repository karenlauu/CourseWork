...
public class Lab2Utilities {
    ...
    * </p>
	...
	public static String mixStrings(String s1, String s2, String s3) {
		...
		if (s1.length() %2 == 0) {
			S1New = s1.substring(0,(s1.length()/2));
		} else {
			S1New = s1.substring(0,s1.length()/2+1);
		}
		
		if (s2.length() >=2) {
			S2New = s2.substring(s2.length()-2);
		} else
			S2New = s2;
		
		if (s3.length()>=2) {
			S3New = s3.substring(0,2);
		}else
			S3New = s3;
		
		return S1New + S2New + S2New + S2New + S3New + S3New + S3New + S3New + S3New;
	}
    ...
    public static String stringCompressor(String inputStr) {
    	...
    	result += "" + inputStr.substring(length -1) + count;
    	return result;
    }
	...
	public static String binaryRepresentation(short value) {
		...
		while (valuee >0) {
			result = (valuee%2) + result;
			valuee = (short) (valuee/2);
		}
		...
		String Strresult = "The decimal value (" + value + ") has binary representation [" + resultt +"]";
		return Strresult;
	}
	...
	public static String binaryXor(short value1 , short value2) {
		...
		String strBin1 = binaryRepresentation(value1);
		int index1 = strBin1.indexOf('[');
		int index2 = strBin1.indexOf(']');
		String Bin1 = strBin1.substring(index1 +1, index2 );
		
		String strBin2 = binaryRepresentation(value2);
		int index3 = strBin2.indexOf('[');
		int index4 = strBin2.indexOf(']');
		String Bin2 = strBin2.substring(index3 +1, index4 );
		
		String result = "";
		for (int i=0; i<16; i++) {
			char char1 = Bin1.charAt(i);
			char char2 = Bin2.charAt(i);
			if (((char1 == '0') && (char2 == '0')) || ((char1 == '1') && (char2 == '1'))) {
				result += "" + 0;
			} else 
				result += "" + 1;
		}String finalResult = "XOR of two values is [" + result +"]";
	...
	public static String countofMult357(int lowerLimit, int upperLimit) {
		...
		String result = "Between (" + lowerLimit + ") and (" + upperLimit +") there are (" + count3 + ") multiple of 3, (" + count5 + ") multiple of 5 and (" + count7 + ") multiple of 7";
